...
str_blur_numbers <- function(
  .str, .rep_cardinal="<NUM>", .rep_ordinal="<ORD>", .lang="en",
  .excl_substr=TRUE, .excl_at_hash=TRUE, .relax_space=TRUE
){
  ...
  if(is.character(.rep_ordinal)){
    ...
    }else if(isTRUE(.excl_at_hash)){
      .ord_regex <- make_regex_excl_at_hash(.ord_regex)
    }
  ...
  if(is.character(.rep_ordinal)){
    ...
    if(isTRUE(.excl_substr)){
      ...
      .card_regex <- make_regex_excl_at_hash(.card_regex)
    }
...
make_cardinal_regex <- function(.lang="en"){
  ...
  .c_start_reg <- paste0(
    "(?:zero|", .c_1_999_reg, "|", .c_big_l_reg, "s?|[0-9][0-9.,]*)"
  )
...
#' @noRd
make_regex_excl_at_hash <- function(.pattern){
